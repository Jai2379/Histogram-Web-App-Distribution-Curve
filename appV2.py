#Jaiaditya's Hisotgram andn Distribution Curve App
...
def store_loaded_data(data, weights=None, custom_bins=None, source_label="raw"):
    """Persist loaded dataset and related metadata."""
...
def _prepare_frequency_arrays(prim_vals, freqs):
    """Shared validation for frequency-based inputs."""
    try:
...
def _determine_replication_counts(freq_ar):
    """Determine how many synthetic samples to draw per frequency entry."""
...
def expand_range_weighted_samples(min_vals, max_vals, freqs):
    """Expand (range start, range end, frequency) rows into sample data."""
    try:
...
def calculate_axis_limits(data):
    """Calculate axis limits with 13% padding."""
    raw_min = np.min(data)
    raw_max = np.max(data)
...
def fit_distribution(dist_obj, dist_name, data):
    """Fit a distribution to data and return fitted parameters."""
    try:
        if dist_name == 'Beta':
...
def get_slider_range(param_name, fitted_value, data):
    """Get reasonable slider range for a parameter."""
...
def plot_histogram(data, mode, selected_dist=None, fitted_params=None, manual_params=None, weights=None, custom_bins=None):
    """Plot histogram with optional distribution overlay."""
    ...
    # Plot histogram with fixed color
...
with st.sidebar:
    ...
    if input_method == "Manual Input":
        ...
        st.caption("Provide the lower/upper bounds for each bin along with its frequency or relative frequency.")
        ...
        uploaded_file = st.file_uploader(
            "Upload a CSV file",
            type=['csv'],
            help="Provide range columns (min, max, frequency)"
        )
...
if st.session_state.data is None:
    ...
    # Histogram customization settings
    ...
    if mode == "View Histogram":
        ...
        # Auto-fit for initial slider values
        dist_obj = DISTRIBUTIONS[selected_dist]
